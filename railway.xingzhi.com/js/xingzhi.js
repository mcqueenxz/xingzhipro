...
Xingzhi.paging = {
	/*
  	 ...
  	 * obj1		数据绑定控件id
  	 */
	show_page:function (total, index, rows, vdate, obj1) {
  		var page = 0;
  		if(total % rows == 0)
    	...
    	if(index <= 0){
        	...
        	htmls += "<a class=\"page-area first-page\" href=\"javascript:;\" onclick=\"getPhotoList(0, "+rows+", '"+vdate+"')\">首页</a>";
        	htmls += "<a class=\"page-area prive-page\" href=\"javascript:;\" onclick=\"getPhotoList("+(index-1)+", "+rows+", '"+vdate+"')\">上一页</a>";        		
    	}
    	htmls += "<a class=\"page-area pagenow\"> " + (index + 1) + " / " + page + " 页</a>";
    	if((index + 1) < page){
    		htmls += "<a class=\"page-area next-page\" href=\"javascript:;\" onclick=\"getPhotoList("+(index+1)+", "+rows+", '"+vdate+"')\">下一页</a>";
    		htmls += "<a class=\"page-area last-page\" href=\"javascript:;\" onclick=\"getPhotoList("+(page-1)+", "+rows+", '"+vdate+"')\">尾页</a>";	
    	} else {
