...
<head>
    ...
    <title><?php echo ($albumname); ?>专辑歌曲列表</title>
...
<body>
    ...
    <div style="padding: 1rem; margin-top: 5rem; padding-bottom: 5rem; overflow: hidden;">
        <ol class="breadcrumb">
            ...
            <li>
                <?php if(is_array($singer)): $i = 0; $__LIST__ = $singer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><a href="/music/albumn_list?sid=<?php echo ($v["id"]); ?>"><?php echo ($v["singername"]); ?>专辑</a><?php endforeach; endif; else: echo "" ;endif; ?>
            </li>
            <li class="active">【<?php echo ($albumname); ?>】歌曲列表</li>
        </ol>
        <ul class="list-inline">
            <li>
                <a href="/music/addsong?sid=<?php echo ($sid); ?>&aid=<?php echo ($aid); ?>"
            ...
            <li>
                <a class="btn btn-primary btn-sm" href="javascript:;"
                   target="_blank" onclick="random_song()">随便听听(<?php echo ($song_total); ?>)</a>
            </li>
            <li>
    ...
    <script type="text/javascript">
        ...
        function get_album() {
            $.ajax("/api/getalbuminfo", {
                ...
                success: function (json) {
                    ...
                    $("#img_thumbnail").attr("src", "<?php echo ($IMGDOMAIN); ?>" + json.result[0].thumbnail);
                },
                error: function (e) {
        ...
        function get_song(vindex, vrows) {
            $.ajax("/api/getsonglist", {
                data: { index: vindex, rows: vrows, albumnId: "<?php echo ($aid); ?>" },
                async: false,
                dataType: "jsonp",
                jsonp: "callback",
                crossDomain: true,
                success: function (json) {
                    ...
                    $.each(json.result, function (i, k) {
                        ...
                        if (k.feat === '' || k.feat === null)
                            shtml += '<td> <?php echo ($v["singername"]); ?> </td>';
                        else
                            shtml += '<td><?php echo ($v["singername"]); ?>/' + Xingzhi.paging.cut_str(k.feat, 4) + '&nbsp;</td>';
                        //作曲
